...
function pageWith(record: Transaction): TransactionPage {
  return {
    items: [record],
    total: 1,
    page: 1,
...
describe('TransactionsPage focused navigation', () => {
  ...
  it('automatically reloads transactions while the page remains open', async () => {
    ...
    expect(await screen.findAllByText('目标产品')).not.toHaveLength(0);
    expect(screen.getByText(/最近同步/)).toBeInTheDocument();
    await act(async () => vi.advanceTimersByTimeAsync(15_000));

    await waitFor(() => expect(transactions).toHaveBeenCalledTimes(2));
    expect(await screen.findAllByText('飞书已修改')).not.toHaveLength(0);
  });
  ...
  it('keeps the last successful records when a background refresh fails', async () => {
    ...
    expect(screen.getAllByText('目标产品')).not.toHaveLength(0);
    fireEvent.click(screen.getByRole('button', { name: '重试同步' }));
    await waitFor(() => expect(apiClient.transactions).toHaveBeenCalledTimes(3));
    expect(screen.getByText(/最近同步/)).toBeInTheDocument();
  });
});
